...
x = infStates(:,1,1); % muhat 1st level
x(r.irr) = []; 
y = r.y(:,1); % responses
y(r.irr) = []; 
...
logx = log(x); % log af x
log1pxm1 = log1p(x-1); % accurate log af (1 + x) - 1
logx(1-x<1e-4) = log1pxm1(1-x<1e-4); % erstatter der hvor vi er tæt på grænseværdierne
log1mx = log(1-x); % same as above 
log1pmx = log1p(-x);
log1mx(x<1e-4) = log1pmx(x<1e-4);

% Calculate log-probabilities for non-irregular trials
reg = ~ismember(1:n,r.irr); % finding regular trials 
