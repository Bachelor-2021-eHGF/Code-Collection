...
%                        contingency. If the agent you are modelling is learning such contingencies,
...
%
%     The choices configured below can be overridden on the command line. Usage then is:
%
...
%DefaultNumberOfDigits=15;

data = load('dataDonda/data24.mat')

r = dataPrep(data.y, data.u);
...
% varargin = ["tapas_hgf_binary_config",...
...
varargin = ["tapas_ehgf_binary_config",...
...
tapas_ehgf_binary_plotTraj(r)
trialLogLls_traj = trialLogLls_fun(r, r.c_prc.prc_fun, r.c_obs.obs_fun, ptrans_prc, ptrans_obs);
...
% --------------------------------------------------------------------------------------------------
function [negLogJoint, negLogLl, rval, err, trialLogLls] = negLogJoint(r, prc_fun, obs_fun, ptrans_prc, ptrans_obs)
...
% the NaNs in infStates are causing the trialLogLls to be NaN. 
...
function [trialLogLls] = trialLogLls_fun(r, prc_fun, obs_fun, ptrans_prc, ptrans_obs)
% Returns the the negative log-joint density for perceptual and observation parameters
...
[negLj, negLl, trialLogLls] = nlj(final);
...
optres.negLj = negLj;
optres.LME = LME;
optres.trialLogLls = trialLogLls;
